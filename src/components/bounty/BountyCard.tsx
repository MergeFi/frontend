...
export function BountyCard({ bounty }: { bounty: Bounty }) {
  ...
  return (
    <Link
      ...
      className={`relative block overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all before:absolute before:inset-y-0 before:start-0 before:w-1 hover:-translate-y-0.5 hover:shadow-md dark:border-slate-800 dark:bg-slate-900 ${accentByDifficulty[bounty.difficulty]}`}
    >
      ...
      <p className="mt-3 line-clamp-2 ps-2 text-sm text-slate-500 dark:text-slate-400">
      ...
      <div className="mt-4 flex items-center gap-4 ps-2 text-xs text-slate-400 dark:text-slate-500">
