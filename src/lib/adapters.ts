...
export function adaptReputation(
  ...
  return {
    ...
    completionRate: snapshot ? coerceFraction(snapshot.completionRate, 100) : 0,
    avgReviewTimeHours: snapshot ? coerceNonNegative(snapshot.avgReviewTimeHours) : 0,
    onTimeDeliveryRate: snapshot ? coerceFraction(snapshot.onTimeDeliveryPercentage, 100) : 0,
